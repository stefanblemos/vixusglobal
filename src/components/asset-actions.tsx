    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {hasAssets && (
        <form action={dedupeAssets} className="ml-auto flex items-center gap-2 text-xs text-slate-500">
          <span className="hidden sm:inline">mantém um de cada (nome/data/custo)</span>
      ...
      {showAdd && (
        <Modal title="Adicionar ativo" onClose={() => setShowAdd(false)}>
          <AssetCreateForm companies={formCompanies} />
        </Modal>
      )}
      {showDetect && (
        <Modal title="Detectar ativos do QBO" onClose={() => setShowDetect(false)}>
