...
// Sub-abas (extrato aprovado 19/07): Visão geral · Extrato (regra da carteira) ·
// Impostos & docs (K-1 por sócio via PoolDocument.memberId). Hoje: admin "ver como
// investidor"; amanhã: a home do portal com login.
...
export default async function InvestorPortfolioPage({
  ...
  if (!p) notFound();
  // tax center: docs pessoais do sócio (K-1/1099/statements) subidos no data room
  const taxDocs =
    vtab === "tax"
        ...
        <div className="mb-2.5 flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-1.5">
            {VTABS.map((tb) => (
              <Link
                key={tb}
                href={`/pools/investors/${key}?tab=${tb}`}
