...
const SEV: Record<Severity, { label: string; dot: string; cls: string }> = {
  alta: { label: "high", dot: "bg-rose-500", cls: "border-rose-200 bg-rose-50" },
  media: { label: "medium", dot: "bg-amber-400", cls: "border-amber-200 bg-amber-50/60" },
  baixa: { label: "low", dot: "bg-slate-300", cls: "border-slate-200 bg-white" },
};
...
export default async function DigestPage({ searchParams }: { searchParams: Promise<{ year?: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-800">Alerts — what needs attention</h1>
        <p className="max-w-3xl text-sm text-slate-500">
          Everything the app found that calls for action, in one place: divergences vs the tax return, missing tax
          returns, state tax without registration, obligations coming due. It&apos;s the content of the{" "}
          <strong>weekly digest</strong> (email delivery is the next step).
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-4">
          ...
          <div className="text-xs text-rose-700">high priority</div>
        </div>
      ...
      {data.alerts.length === 0 ? (
        <div className="rounded-xl border border-[#8DC63F]/50 bg-[#8DC63F]/10 p-6 text-sm text-[#3B6D11]">
          ✓ Nothing pending in {year}. Everything the app checks is in order.
        </div>
      ) : (
      ...
      <p className="text-[11px] text-slate-400">
        Next step (infra): deliver this via automatic weekly email — when QBO sends the reports,
        the app imports, reconciles, and sends this summary. Requires setting up the email service + cron.
      </p>
    </div>
  );
}
